...
# Function to fetch known faces from the database
def fetch_known_faces(connection):
    cursor = connection.cursor()
    cursor.execute("SELECT username, photo FROM user")
    ...
    for row in rows:
        ...
        face_encoding_str = row[1].replace("[", "").replace("]", "").replace(" ", "")
        face_encoding_list = [float(val) for val in face_encoding_str.split(",")]
        known_face_encodings.append(face_encoding_list)
    cursor.close()
    return known_face_names, known_face_encodings


#capture the video from default camera 
file_video_stream = cv2.VideoCapture('screenshot/modi.mp4')

# Load known faces from the database
known_face_names, known_face_encodings = fetch_known_faces(connection)
